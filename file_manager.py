...
highlighted_item, path, last_path, copy_path, current_page, items_pages, delete_warning, preview_file, preview_len, preview_line = use_cfg[1]
...
while True:
    ...
    render = sf.render_things(menu, list_len, preview_file, preview_line, lower_bar, max_name_len, show_hidden_items, highlighted_item, max_list_len, items_pages, current_page, first_index, last_index)
    ...
    if not preview_file:
        ...
        current_page += (sf.change_page(key, highlighted_item, current_page, items_pages, list_len, first_index, last_index, delete_warning))
        deleting_item = sf.delete_item(lower_bar, path, last_path, highlighted_item, list_len, delete_warning) if key == 68 else [0, False, ""]
